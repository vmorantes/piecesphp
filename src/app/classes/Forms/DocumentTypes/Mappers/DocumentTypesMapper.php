 ...
 * DocumentTypesMapper.
 *
 * @package     Forms\DocumentTypes\Mappers
...
class DocumentTypesMapper extends EntityMapperExtensible
{
    ...
    const TABLE = 'forms_document_types';
    ...
    {
        if (self::existsByName($this->documentTypeName, -1)) {
            throw new DuplicateException(__(self::LANG_GROUP, "Ya existe un tipo de documento con el nombre '{$this->documentTypeName}'."));
        }
    ...
    {
        if (self::existsByName($this->documentTypeName, $this->id)) {
            throw new DuplicateException(__(self::LANG_GROUP, "Ya existe un tipo de documento con el nombre '{$this->documentTypeName}'."));
        }
     ...
     *
     * @param \stdClass|DocumentTypesMapper|int $elementOrID
     * @param string $lang
     * @return string
     */
    ...
    public static function allForSelect(string $defaultLabel = '', string $defaultValue = '', bool $onlyActives = true)
    {
        ...
        /**
         * @param DocumentTypesMapper $e
         */
        array_map(function ($e) use (&$options, &$onlyActives) {

            $allow = true;

            if ($onlyActives) {
                if ($e->status == DocumentTypesMapper::STATUS_ACTIVE) {
                    $allow = true;
                } else {
                    $allow = false;
                }
    ...
    {
        return (new DocumentTypesMapper)->getModel();
    }
}
