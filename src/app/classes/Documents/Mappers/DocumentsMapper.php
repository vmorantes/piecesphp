 ...
 * @property int|DocumentTypesMapper $documentType
...
class DocumentsMapper extends EntityMapperExtensible
{
    ...
    protected $fields = [
        ...
        'documentType' => [
            'type' => 'int',
            'reference_table' => DocumentTypesMapper::TABLE,
            'reference_field' => 'id',
            'reference_primary_key' => 'id',
            'human_readable_reference_field' => 'id',
            'mapper' => DocumentTypesMapper::class,
        ],
        'documentName' => [
            'type' => 'text',
        ],
    ...
    const TABLE = 'documents_elements';
    ...
    {
        if (self::existsByDocumentName($this->documentName, -1)) {
    ...
    {
        if (self::existsByDocumentName($this->documentName, $this->id)) {
            //throw new DuplicateException(__(self::LANG_GROUP, "Ya existe un documento con ese nombre"));
        }
        if (!$noDateUpdate) {
     ...
     *
     * @param \stdClass|DocumentsMapper|int $elementOrID
     * @param string $lang
     * @return string
     */
     ...
     * @return int
     */
    public static function countAll(bool $onlyActives = true)
    {
        $model = self::model();
        $model->select("COUNT(id) AS total");

        if ($onlyActives) {
            $model->where("status = " . self::STATUS_ACTIVE);
        }
    ...
    {
        return (new DocumentsMapper)->getModel();
    }
}
