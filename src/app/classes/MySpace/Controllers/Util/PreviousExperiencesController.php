...
/**
 * PreviousExperiencesController.php
 */
...
/**
 * PreviousExperiencesController.
 *
 * @package     MySpace\Controllers\Util
 * @author      Vicsen Morantes <sir.vamb@gmail.com>
 * @copyright   Copyright (c) 2025
 */
class PreviousExperiencesController extends AdminPanelController
{
    ...
    public function dataTables(Request $request, Response $response)
    {
        ...
        $table = PreviousExperiencesMapper::TABLE;
        $whereString = null;
        $havingString = null;
        $and = 'AND';

        $where = [
            "{$table}.profile = {$profileID}",
        ];
        $having = [];
        ...
        $selectFields = PreviousExperiencesMapper::fieldsToSelect();
        ...
        foreach ($rawData as $index => $element) {
            ...
            $mapper = new PreviousExperiencesMapper($element->id);

            $rawData[$index] = $this->render(
                'util/experience-list-card',
                [
                    'mapper' => $mapper,
                    'deleteRoute' => MyProfileController::routeName('actions-delete-experience', ['id' => $mapper->id]),
                    'hasDelete' => MyProfileController::allowedRoute('actions-delete-experience', ['id' => $mapper->id]),
    ...
    public function toDelete(Request $request, Response $response, array $args)
    {
        ...
        try {
            ...
            try {
                ...
                $exists = PreviousExperiencesMapper::existsByID($id);
                ...
                if ($exists) {
                    ...
                    $pdo = PreviousExperiencesMapper::model()->getDatabase();
