...
class NewsCategoryMapper extends EntityMapperExtensible
{
     ...
     *
     * @param \stdClass|NewsCategoryMapper|int $elementOrID
     * @param string $lang
     * @return string
     */
    ...
    /**
     * Crea u obtiene la categoría genérica para las noticias huérfanas por la eliminación
     *
    ...
    public static function uncategorizedCategory()
    {
        ...
        $category = new NewsCategoryMapper(self::UNCATEGORIZED_ID);

        if ($category->id == null) {
            $category->setLangData(Config::get_default_lang(), 'name', 'Noticia');
            $category->setLangData('es', 'name', 'Noticia');
            $category->setLangData('en', 'name', 'News');
            $category->color = get_config('main_brand_color');
            $category->iconImage = self::DEFAULT_ICON;
    ...
    {
        return (new NewsCategoryMapper)->getModel();
    }
}
