...
CliActions::make("{$cliTaskName}:{$cliTaskFlag}", function ($args) {
    ...
    echoTerminal('[1/20]WhereSegment deja la comilla FUERA del SQL');
    ...
    //──── 2. La discriminante ───────────────────────────────────────────────────────────
    echoTerminal('[2/20]DISCRIMINANTE: la vía de cadena mete la comilla en el SQL');
    ...
    echoTerminal('[3/20]HavingSegment deja la comilla FUERA del HAVING');
    ...
    echoTerminal('[4/20]Las búsquedas arregladas siguen por la vía parametrizada');
    ...
    echoTerminal('[5/20]Las cuatro listas `IN (...)` siguen validando el dominio');
    ...
    echoTerminal('[6/20]El desplegable de usuarios: `having` preparado y `NOT IN` validado');
    ...
    echoTerminal('[7/20]Los fragmentos `where_string`/`having_string` validan su dominio');
    ...
    echoTerminal('[8/20]`where_segment` prepara, y sin él la vía de cadena sigue intacta');
    ...
    echoTerminal('[9/20]El SQL de `process()` con segmento se EJECUTA de verdad');
    ...
    echoTerminal('[10/20]El desplegable de aprobaciones manda el CRUDO en cualquier idioma');
    ...
    echoTerminal('[11/20]El grupo de búsqueda se une con AND, no con OR');
    ...
    echoTerminal('[12/20]`AllProfiles` parentiza, y `processFromQuery` va por marcador');
    ...
    echoTerminal('[13/20]La dirección de `custom_order` se normaliza a ASC o DESC');
    ...
    echoTerminal('[14/20]Los listados paginados (`PageQuery`) mandan la petición por marcador');
    ...
    echoTerminal('[15/20]Los usos de escapeString() van por marcador, y no reaparecen');
    ...
    echoTerminal('[16/20]sqlStringLiteral(): las etiquetas del SERVIDOR entran en el SELECT sin depender de sql_mode');
    ...
    echoTerminal('[17/20]process(): sin having_string, el buscador va por marcador; con contenido, sigue por cadena');
    ...
    echoTerminal('[18/20] Aprobaciones y LoginAttempts: el HAVING va por marcador, y nadie pasa having_string');
    ...
    $havingDe = static fn (string $sql): string => (string) mb_substr($sql, (int) mb_strpos($sql, 'HAVING'));
    $previoUsuario = get_config('current_user');
    $previoGuardado = get_config('pcsphp_current_user_stored');
    try {
        //El usuario 1 existe en toda instalación. Su paquete no escribe: la fila TOTP ya la creó el arranque.
        set_config('current_user', (object) ['id' => 1]);
        getLoggedFrameworkUser(true);
        $respuesta = (new \SystemApprovals\Controllers\SystemApprovalsController())->dataTables($peticionListado([
            'draw' => 1, 'start' => 0, 'length' => 10, 'elapsedDays' => '0',
            'columns' => array_fill(0, 4, ['searchable' => 'true']),
            'search' => ['value' => $conComilla, 'regex' => 'false'],
        ]), new \PiecesPHP\Core\Routing\ResponseRoute());
        $valores = json_decode((string) $respuesta->getBody(), true);
        $having = $havingDe((string) ($valores['SQL_MAIN_EXECUTED'] ?? ''));
        $check(mb_strpos($having, ':WH') !== false, 'aprobaciones: el HAVING lleva marcadores', $having);
        $check(mb_strpos($having, "'" . \App\Model\UsersModel::TABLE . "'") === false && mb_strpos($having, "'" . \SystemApprovals\Mappers\SystemApprovalsMapper::STATUS_APPROVED . "'") === false,
            'aprobaciones: ni la tabla de usuarios ni APPROVED van como literal en el HAVING');
    ...
    echoTerminal('[19/20] getUserDataByUsername() y existsByName(): el valor de la petición viaja como dato');
    ...
    //──── 20. getByID() y existsByID() de cuatro mappers, por marcador (#241) ──────────────
    echoTerminal('[20/20] getByID() y existsByID() de Point, Documents, Categories y DocumentTypes van por WhereSegment');
    //El SQL lo arma y ejecuta el propio método estático: su modelo no sale. Se mira la FUENTE por tokens,
    //y el comportamiento con un id que existe y con uno que no.
    $cuerpoTokens = function (string $clase, string $metodo): array {
        ...
        return array_values(array_filter(token_get_all('<?php ' . $cuerpo), fn($t) => !is_array($t) || !in_array($t[0], [\T_WHITESPACE, \T_COMMENT, \T_DOC_COMMENT, \T_OPEN_TAG], true)));
    };
    $porMarcador = function (array $tokens): bool {
        $texto = implode('', array_map(fn($t) => is_array($t) ? $t[1] : $t, $tokens));
        $interpolaId = false;
        foreach ($tokens as $t) {
            if (is_array($t) && in_array($t[0], [\T_ENCAPSED_AND_WHITESPACE, \T_CONSTANT_ENCAPSED_STRING], true) && preg_match('/\bid\s*=/', $t[1]) === 1) {
                $interpolaId = true;
            }
        }
        return str_contains($texto, "where(newWhereSegment([newWhereItem('id',WhereItem::EQUAL_OPERATOR,\$id)]))") && !$interpolaId;
    };
    $objetivos = [
        [\App\Locations\Mappers\PointMapper::class, ['getByID']],
        [\Documents\Mappers\DocumentsMapper::class, ['getByID', 'existsByID']],
        [\Forms\Categories\Mappers\CategoriesMapper::class, ['getByID', 'existsByID']],
        [\Forms\DocumentTypes\Mappers\DocumentTypesMapper::class, ['getByID', 'existsByID']],
    ];
    try {
        foreach ($objetivos as [$clase, $metodos]) {
            $corto = substr($clase, strrpos($clase, '\\') + 1);
            foreach ($metodos as $metodo) {
                $check($porMarcador($cuerpoTokens($clase, $metodo)), "{$corto}::{$metodo}(): where() recibe WhereSegment con el id, sin cadena «id =»");
            }
            $modelo = $clase::model();
            $modelo->select('MIN(id) AS minimo, MAX(id) AS maximo')->execute();
            $filas = $modelo->result();
            $minimo = is_array($filas) && isset($filas[0]->minimo) ? (int) $filas[0]->minimo : 0;
            $maximo = is_array($filas) && isset($filas[0]->maximo) ? (int) $filas[0]->maximo : 0;
            $inexistente = $maximo + 1000;
            if ($minimo > 0) {
                $fila = $clase::getByID($minimo);
                $check(is_object($fila) && (int) $fila->id === $minimo, "{$corto}::getByID({$minimo}) devuelve esa fila");
                if (in_array('existsByID', $metodos, true)) {
                    $check($clase::existsByID($minimo) === true, "{$corto}::existsByID({$minimo}) es true");
                }
            } else {
                echoTerminal("      - {$corto}: tabla vacía en esta base; solo se prueba el id inexistente");
            }
            $check($clase::getByID($inexistente) === null, "{$corto}::getByID({$inexistente}) da null");
            if (in_array('existsByID', $metodos, true)) {
                $check($clase::existsByID($inexistente) === false, "{$corto}::existsByID({$inexistente}) es false");
            }
        }
    } catch (\Throwable $e) {
        $check(false, 'getByID()/existsByID() se ejecutan', 'EXCEPCIÓN: ' . mb_substr($e->getMessage(), 0, 200));
    }
    echoTerminal(' ');
