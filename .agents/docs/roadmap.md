...
| 7c | **E4 · ventana de correo** | Pruebas de los 10 envíos, en tres capas: composición sin red, sumidero SMTP local y entrega real revisada a mano | `18` T7; ADR 0015 | Parte A y B1 cerradas (`#126`→`#142`: recuperación por código, envíos 6, 8 y 9, escape de las plantillas públicas); B2a cerrada (`#166`: comentario de token con escape y OTP); B2b cerrada (`#168`, contacto); B2c cerrada (`#172`: aprobaciones y alta por API, con escape del nombre); B3 en curso (`#173`, Mailinator) y B3 (Mailinator, ADR 0011). La capa 1 queda absorbida en la 2 salvo el escape de plantillas |
