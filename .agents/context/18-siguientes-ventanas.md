...
> a los documentos numerados, lo demás baja a [`historico/`](./historico/), y **git guarda la
> procedencia** — nada se pierde, cambia de sitio.
>
> **Mientras exista NO ES MATERIAL DE ATERRIZAJE.** Quien viene a USAR el framework no pasa por
> aquí: empieza por [`README.md`](./README.md) y sigue por `01`–`15`. Esto es de mantenedor.
>
> **Las leyes ya no están aquí**: viven en [19-leyes.md](./19-leyes.md), que **no** se disuelve.
> Estaban dentro de `T0` —un contenedor temporal— y por eso este archivo no podía morir.
